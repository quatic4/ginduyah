...
export function previewSnapshot(today: string): Snapshot {
  ...
  const data: Snapshot = {
    workspace: { id: "preview", name: "The upload crew", owner_id: "preview-you", invite_code: null },
    members: [{ user_id: "preview-you", display_name: "You", active: true }, { user_id: "preview-friend", display_name: "Teammate", active: true }],
    channels: [
      { id: "preview-ginduyah", workspace_id: "preview", name: "ginduyah", handle: "@ginduyah", comic_goal: 2, post_goal: 1 },
      { id: "preview-nba", workspace_id: "preview", name: "NBA comics", handle: "", comic_goal: 1, post_goal: 0 },
...
export function mutatePreview(data: Snapshot, operation: Operation, payload: Record<string, unknown>): Snapshot {
  ...
  if (operation === "add_item") {
    next.items.push({ id, channel_id: String(payload.channel_id), due_date: String(payload.due_date), title: String(payload.title), content_type: payload.content_type as "comic" | "internet_post", link: String(payload.link || ""), notes: String(payload.notes || ""), submitted_by: "preview-you", created_at: now, version: 1 });
    STAGES.forEach(stage => {
      ...
      next.steps.push({ item_id: id, stage: stage.id, assigned_to: null, completed_by: done ? "preview-you" : null, completed_at: done ? now : null, version: 1 });
    });
    ...
    if (step) {
      if ("assigned_to" in payload) step.assigned_to = payload.assigned_to ? String(payload.assigned_to) : null;
        ...
        step.completed_by = payload.done ? "preview-you" : null; step.completed_at = payload.done ? now : null;
      }
      step.version++;
    }
  } else if (operation === "set_goal") {
    next.goals = next.goals.filter(g => !(g.channel_id === payload.channel_id && g.day === payload.day));
    next.goals.push({ channel_id: String(payload.channel_id), day: String(payload.day), comic_goal: Number(payload.comic_goal), post_goal: Number(payload.post_goal) });
  } else if (operation === "add_channel") {
    next.channels.push({ id, workspace_id: "preview", name: String(payload.name), handle: String(payload.handle || ""), comic_goal: Number(payload.comic_goal), post_goal: Number(payload.post_goal) });
  } else if (operation === "edit_channel") {
    const channel = next.channels.find(c => c.id === payload.channel_id);
    if (channel) Object.assign(channel, { name: payload.name, handle: payload.handle });
  } else if (operation === "remove_member" || operation === "restore_member") {
    const member = next.members.find(m => m.user_id === payload.user_id);
    if (member) { member.active = operation === "restore_member"; title = member.display_name; }
  }
  const actions: Record<string, string> = { add_item: "submitted", edit_item: "updated", delete_item: "removed", set_goal: "changed targets for", add_channel: "added channel", edit_channel: "updated channel", remove_member: "removed access for", restore_member: "restored access for" };
  next.activity.unshift({ id: crypto.randomUUID(), channel_id: item?.channel_id || (payload.channel_id as string) || null, item_id: item?.id || null, actor_id: "preview-you", actor_name: "You", action: operation === "set_step" ? ("assigned_to" in payload ? "changed the assignment for" : payload.done ? "completed" : "reopened") : actions[operation] || operation, title, stage: operation === "set_step" ? payload.stage as typeof STAGES[number]["id"] : null, created_at: now });
  return next;
}
